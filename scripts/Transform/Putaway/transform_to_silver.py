...
def extract_transform_print(file_name, dw_file_path):
    print('FILE NAME:', file_name) #PUTAWAY-2024-08-14-12.csv
    ...
    try:
        ...
        df.iloc[:, 8].fillna(datetime(1500, 1, 11, 11, 11, 11), inplace=True)

        df["sector"] = "putaway"
        df["current_date_"] = datetime.now().strftime("%Y-%m-%d")
        df.fillna('-', inplace=True)

        hours = df.iloc[0, 8]  # completion time
...
def join_df(file_name, df_dwb, df_dwd):
    ...
    delimiter = ';'
    output_path = os.path.join('/opt/airflow/data_lake/Silver/Putaway', file_name)
    ...
    os.remove(os.path.join('/opt/airflow/data_lake/Bronze/WHB/Putaway', file_name))
    os.remove(os.path.join('/opt/airflow/data_lake/Bronze/WHD/Putaway', file_name))
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        whb_filepath = '/opt/airflow/data_lake/Bronze/WHB/Putaway'
        whd_filepath = '/opt/airflow/data_lake/Bronze/WHD/Putaway'
