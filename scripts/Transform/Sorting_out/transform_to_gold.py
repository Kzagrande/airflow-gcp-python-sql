...
def transform_to_gold(file_name):
    ...
    file_path = os.path.join('/opt/airflow/data_lake/Silver/Sorting_out', file_name)
    ...
    try:
        ...
        df = df.iloc[:, [0,5, 12,13, 14, 15, 15, 17  ]]
        ...
        df['sorting_time_next'] = df.groupby('operated_by')['operation_time'].shift(-1)
        df['sorting_time_next'] = df['sorting_time_next'].fillna(df['operation_time'])
        df['operation_time'] = pd.to_datetime(df['operation_time'])
        df['sorting_time_next'] = pd.to_datetime(df['sorting_time_next'])

        # Calcular a diferença em segundos entre 'operation_time' e 'sorting_time_next'
        df['effective_hours'] = (df['sorting_time_next'] - df['operation_time']).dt.total_seconds() 
        ...
        output_path = os.path.join('/opt/airflow/data_lake/Gold/Sorting_out', file_name)
