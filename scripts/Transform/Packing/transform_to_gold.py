...
def transform_to_gold(file_name):
    ...
    file_path = os.path.join('/opt/airflow/data_lake/Silver/Packing', file_name)
    ...
    try:
        ...
        df['packing_time_next'] = df.groupby('operated_by')['operation_time'].shift(-1)
        df['packing_time_next'] = df['packing_time_next'].fillna(df['operation_time'])
        df['operation_time'] = pd.to_datetime(df['operation_time'])
        df['packing_time_next'] = pd.to_datetime(df['packing_time_next'])

        # Calcular a diferença em segundos entre 'operation_time' e 'packing_time_next'
        df['effective_hours'] = (df['packing_time_next'] - df['operation_time']).dt.total_seconds() 
        ...
        output_path = os.path.join('/opt/airflow/data_lake/Gold/Packing', file_name)
