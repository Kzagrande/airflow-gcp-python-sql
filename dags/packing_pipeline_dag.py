...
def get_file_name(**kwargs):
    ...
    file_name = f'PACKING-{current_time.strftime("%Y-%m-%d-%H.xlsx")}'
...
def download_file_d(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHD/Packing', file_name)
    
    download_task = GCSToLocalFilesystemOperator(
        task_id='download_file_D',
        bucket='wms-uph-pipeline',
        object_name=f'Bronze/WHD/Packing/D-{file_name}',
...
def download_file_b(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHB/Packing', file_name)
    
    download_task = GCSToLocalFilesystemOperator(
        task_id='download_file_B',
        bucket='wms-uph-pipeline',
        object_name=f'Bronze/WHB/Packing/B-{file_name}',
        filename=local_file_path,
        gcp_conn_id='google-cloud-storage',
        trigger_rule='all_success'
    )
...
def upload_silver_to_gcs(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Silver/Packing', file_name)
    
    upload_task = LocalFilesystemToGCSOperator(
        task_id='upload_to_silver_gcs_task',
        bucket='wms-uph-pipeline',
        dst=f'Silver/Packing/{file_name}',
        src=local_file_path,
        gcp_conn_id='google-cloud-storage'
    )
...
with DAG(
    ...
    transform_to_silver = BashOperator(
        ...
        bash_command="python3 /opt/airflow/scripts/Transform/Packing/transform_to_silver.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dw_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Packing/load_to_dw.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )

    transform_to_gold = BashOperator(
        task_id='transform_to_gold',
        bash_command="python3 /opt/airflow/scripts/Transform/Packing/transform_to_gold.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dataset_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Packing/load_to_dataset.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
