...
def get_file_name(**kwargs):
    ...
    file_name = f'PICKING-{current_time.strftime("%Y-%m-%d-%H.xlsx")}'
...
def download_file_d(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHD/Picking', file_name)
...
def download_file_b(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHB/Picking', file_name)
...
with DAG(
    ...
    transform_to_silver = BashOperator(
        ...
        bash_command="python3 /opt/airflow/scripts/Transform/Picking/transform_to_silver.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dw_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Picking/load_to_dw.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )

    transform_to_gold = BashOperator(
        task_id='transform_to_gold',
        bash_command="python3 /opt/airflow/scripts/Transform/Picking/transform_to_gold.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dataset_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Picking/load_to_dataset.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
