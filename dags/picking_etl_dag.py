...
with DAG(
    ...
    transform_to_gold = BashOperator(
        ...
        bash_command="python3 /opt/airflow/scripts/Transform/Picking/transform_to_gold.py {{ ti.xcom_pull(task_ids='get_file_name', key='file_name') }}"
    )
    ...
    upload_to_dataset_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Picking/load_to_dataset.py {{ ti.xcom_pull(task_ids='get_file_name', key='file_name') }}"
    )

    # Definição das dependências
    get_file_name_task >> download_file_task >> transform_to_silver
