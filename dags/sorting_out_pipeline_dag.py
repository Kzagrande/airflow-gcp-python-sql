...
def get_file_name(**kwargs):
    ...
    file_name = f'SORTING-{current_time.strftime("%Y-%m-%d-%H.xlsx")}'
...
def download_file_d(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHD/Sorting_out', file_name)
    
    download_task = GCSToLocalFilesystemOperator(
        task_id='download_file_D',
        bucket='wms-uph-pipeline',
        object_name=f'Bronze/WHD/Sorting_out/D-{file_name}',
...
def download_file_b(**kwargs):
    ...
    local_file_path = os.path.join('/opt/airflow/data_lake/Bronze/WHB/Sorting_out', file_name)
    
    download_task = GCSToLocalFilesystemOperator(
        task_id='download_file_B',
        bucket='wms-uph-pipeline',
        object_name=f'Bronze/WHB/Sorting_out/B-{file_name}',
        filename=local_file_path,
        gcp_conn_id='google-cloud-storage',
        trigger_rule='all_success'
    )
...
def upload_silver_to_gcs(**kwargs):
    ...
    upload_task = LocalFilesystemToGCSOperator(
        ...
        bucket='wms-uph-pipeline',
        dst=f'Silver/Sorting_out/{file_name}',
        src=local_file_path,
        gcp_conn_id='google-cloud-storage'
    )
...
def upload_gold_to_gcs(**kwargs):
    ...
    upload_task = LocalFilesystemToGCSOperator(
        ...
        bucket='wms-uph-pipeline',
        dst=f'Gold/Sorting_out/{file_name}',
        src=local_file_path,
        gcp_conn_id='google-cloud-storage'
    )
    upload_task.execute(context=kwargs)
...
with DAG(
    ...
    transform_to_silver = BashOperator(
        ...
        bash_command="python3 /opt/airflow/scripts/Transform/Sorting_out/transform_to_silver.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dw_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Sorting_out/load_to_dw.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )

    transform_to_gold = BashOperator(
        task_id='transform_to_gold',
        bash_command="python3 /opt/airflow/scripts/Transform/Sorting_out/transform_to_gold.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
    ...
    upload_to_dataset_task = BashOperator(
       ...
       bash_command="python3 /opt/airflow/scripts/Load/Sorting_out/load_to_dataset.py {{ ti.xcom_pull(task_ids='get_file_name_task', key='file_name') }}"
    )
